...
- 6d53b2c: Fix ghost Buttons turning invisible on hover.
...
- 560d57a: Document two silent theming gotchas found while integrating the library into a consumer app.
...
- 45de965: Fix a broken CSS import, missing dependency, and misleading peer-dep metadata; correct several inaccurate docs.
...
- 742ef26: Fix: the precompiled `styles.css` now includes the full `tailwindcss-animate`
...
- 47b6ce2: Ship a precompiled stylesheet and fix dynamically-built layout classes.
...
- 7334533: feat(ui): InfinityAnim speed tiers (+33% active, 25% idle)
...
- 0dea2c2: `InfinityAnim`: add `palette` prop and smart `runWhile` pause/resume.
...
- 6f5cc0a: Add Line variant to Tabs with an animated, spring-driven underline that glides between the active tab. The variant is opt-in via `<TabsList variant="line">` and flows to the width of its container (drop it into a sidebar, card, or main column without resizing).
...
- 12c5462: Fix KnowledgeGraph nodes not rendering (blank canvas with labels only).
...
- 499e2f5: Add `@opencosmos/ui/knowledge-graph` subpath export — sigma.js v3 WebGL constellation renderer for the OpenCosmos knowledge graph.
...
- 077f48c: Migrate to Tailwind CSS v4.0.0. Fix documentation 404 for Design Tokens Foundations page.
...
- cca136a: Migrate to Tailwind CSS v4.0.0. Fix documentation 404 for Design Tokens Foundations page.
